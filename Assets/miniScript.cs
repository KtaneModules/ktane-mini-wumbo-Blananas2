...
public class miniScript : MonoBehaviour
{
    ...
    void SwitchToggle(KMSelectable swithc)
    { //misspelled so C# doesn't go insane
...
#pragma warning restore 0414
    ...
    private IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        if (_currentPanel == 5)
        {
            ...
            if (m.Success)
            {
                var posNames = new string[] { "up", "left", "all", "right", "down" };
            ...
            if (m.Success)
            {
                var posNames = new string[] { "up", "left", "middle", "right", "down" };
                var ix = Array.IndexOf(posNames, parameters[1]);
                if (ix == -1)
                    yield break;
                yield return null;
